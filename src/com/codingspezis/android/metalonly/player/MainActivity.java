...
public class MainActivity extends SherlockActivity implements OnClickListener, OnItemClickListener{
	...
	private void setUpGUIObjects(){
		buttonStream = (ImageButton) findViewById(R.id.imageButton2);
		...
		buttonCalendar = (ImageButton) findViewById(R.id.imageButton1);
		buttonCalendar.setOnClickListener(this);
		buttonWish = (ImageButton) findViewById(R.id.imageButton3);
		buttonWish.setOnClickListener(this);
		toggleStreamButton(false);
		marqueeMod = (Marquee)findViewById(R.id.marquee1);
		marqueeGenre = (Marquee)findViewById(R.id.marquee2);
	...
	private void displaySongs(){
		...
		for(int i=historySaver.size()-1; i>=0; i--){
		    ...
		    dataEntry.put(SongAdapter.KEY_INTERPRET, historySaver.get(i).interpret);
		    dataEntry.put(SongAdapter.KEY_THUMB, historySaver.get(i).thumb);
		    dataEntry.put(SongAdapter.KEY_DATE, longToDateString(historySaver.get(i).date));
		    data.add(dataEntry);
		}
	 ...
	 * @param listening if this value is true the background color of the stream button is set to green
	 */
	private void toggleStreamButton(boolean listening){
		if(listening){
			buttonStream.setImageDrawable(getResources().getDrawable(R.drawable.mo_stop5));
		}else{
			buttonStream.setImageDrawable(getResources().getDrawable(R.drawable.mo_play5));
		}
	}
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		...
		fav.setIcon(R.drawable.ic_rating_important_dark);
	...
	private void startPlanActivity(){
		PlanGrabber pg = new PlanGrabber(this, "http://www.metal-only.de/botcon/mob.php?action=plan");
		pg.start();
	}
			...
			public void onWishesChecked(AllowedActions allowedActions) {
				if(allowedActions.moderated){
					if(allowedActions.wishes || allowedActions.regards){
						...
						alertMessage(mainActivity, mainActivity.getString(R.string.no_wishes_and_regards));
					}
				}else{
	...
	private void showVolumeBar(){
		...
		input.setProgress(audioManager.getStreamVolume(AudioManager.STREAM_MUSIC));
	 ...
	 * sends start intent to PlayerService
	 ...
	 * sends stop intent to PlayerService
	 */
	...
	/**
	 * displays meta data
	 * @param metadata data to display
	 */
	...
	private void handleAction(final int index, int action){
		...
		case 0: // add to favorites
			...
			break;
		case 1: // YouTube
			String searchStr = historySaver.get(index).interpret+" - "+historySaver.get(index).title;
			try {
			...
			break;
		case 2: // share
			String message = historySaver.get(index).interpret+" - "+historySaver.get(index).title;
	...
	public static class MetadataParser{
		...
		public Song toSong(){ 
			...
			if(song.thumb.indexOf(" OnAir")>0)
				song.thumb = song.thumb.substring(0, song.thumb.indexOf(" OnAir")).trim();
	 ...
	 * @version 25.12.2012
	 * 
	 * broadcast receiver class for communication between other activities or services
	 * 
	 */
		...
		public void onReceive(Context context, Intent intent) {
			...
			if(intent.getAction().equals(PlayerService.INTENT_STATUS)){ 
				...
				if(intent.getBooleanExtra(PlayerService.EXTRA_CONNECTED, false)){
					shouldPlay = true;
					toggleStreamButton(true);
					metadataParser = new MetadataParser(intent.getStringExtra(PlayerService.EXTRA_META));
					displayMetadata();
				}else{ 
					stopListening();
					toggleStreamButton(false);
	...
	private class PlanGrabber {
		...
		private OnHTTPGrabberListener listener =
